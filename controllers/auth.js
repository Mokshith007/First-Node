...
const transporter=nodemailer.createTransport(sendgridTransport({
  auth:{
    api_key:'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'
  }
}));
...
exports.postSignup = (req, res, next) => {
  ...
  .then(userDoc=>{
    if(userDoc){
      req.flash('error','email exists already use another')
  ...
  exports.postReset=(req,res,next)=>{
    crypto.randomBytes(32,(err,buffer)=>{
      ...
      .then((user)=>{
          ...
          user.restTokenExpiration=Date.now()+600000
          return user.save()
      })
      .then(result=>{
        res.redirect("/")
